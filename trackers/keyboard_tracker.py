...
# store key press
    ...
    def get_metrics(self):
        ...
        if self.pause_times:
            avg_pause = round(sum(self.pause_times) / len(self.pause_times), 2)

        if len(self.pause_times) >= 2:
            variance = round(statistics.stdev(self.pause_times), 2)

        long_pause_count = len([p for p in self.pause_times if p > 5])
